...
# for some reason, font colour remains black if using the color option
    dbc.Row([
        dbc.Col(dbc.Card(html.H3(children='Total Cases in Country',id="lab1",
    ...
    dbc.Row([
        dbc.Col(dcc.Graph(id='cases')),
        # dbc.Col(dcc.Graph(id='line_cases_or_deaths'), width=6)
        ]),

        dbc.Row([
            dbc.Col(dbc.Card(html.H3(children='Total Deaths in Country: ',id="lab2",
                                     className="text-center text-light bg-dark"), body=True, color="dark")
                    , className="mb-4")
        ]),
        # dbc.Row([
...
@app.callback(
Output("lab1","children"),
    [Input("count","value")]
)
def total_cases(count):
    df1 = df.loc[(df['location'] == count)]
    last_row = df1.iloc[[-1]]
...
@app.callback(
Output("lab2","children"),
    [Input("count","value")]
)
def total_cases(count1):
    df1 = df.loc[(df['location'] == count1)]
    last_row = df1.iloc[[-1]]
